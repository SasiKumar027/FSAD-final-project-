...
function AdminDashboard() {
  ...
  const pendingApps = applications.filter((a) => a.status === STATUS.PENDING).length;
  const approvedApps = applications.filter((a) => a.status === STATUS.APPROVED).length;
  const rejectedApps = applications.filter((a) => a.status === STATUS.REJECTED).length;
  ...
  const sidebarLinks = [
    ...
    { path: "applications", label: "View Applications", icon: "📝" }
  ];
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100%", flexDirection: "column" }}>
      <div style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", color: "white", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", boxShadow: "0 4px 12px rgba(0, 0, 0, 0.15)", flexShrink: 0 }}>
        ...
        <div style={{ fontSize: "clamp(1.2em, 5vw, 1.5em)", fontWeight: "bold" }}>👨‍💼 Work-Study Admin</div>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, padding: "24px", background: "#f9fafb", overflowY: "auto", maxWidth: "100%" }}>
          <div style={{ marginBottom: "24px" }}>
            ...
            <p style={{ margin: "4px 0 0 0", color: "#6b7280", fontSize: "clamp(0.9em, 3vw, 1em)" }}>System overview and recent activity</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "16px", marginBottom: "24px" }}>
            <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: "0.85em", color: "#6b7280", marginBottom: "8px", fontWeight: "600" }}>Total Jobs</div>
              <div style={{ fontSize: "clamp(1.8em, 8vw, 2.5em)", fontWeight: "bold", color: "#10b981" }}>{totalJobs}</div>
            </div>
            <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: "0.85em", color: "#6b7280", marginBottom: "8px", fontWeight: "600" }}>Total Applications</div>
              <div style={{ fontSize: "clamp(1.8em, 8vw, 2.5em)", fontWeight: "bold", color: "#3b82f6" }}>{totalApps}</div>
            </div>
            <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: "0.85em", color: "#6b7280", marginBottom: "8px", fontWeight: "600" }}>Pending</div>
              <div style={{ fontSize: "clamp(1.8em, 8vw, 2.5em)", fontWeight: "bold", color: "#f59e0b" }}>{pendingApps}</div>
            </div>
            <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: "0.85em", color: "#6b7280", marginBottom: "8px", fontWeight: "600" }}>Approved</div>
              <div style={{ fontSize: "clamp(1.8em, 8vw, 2.5em)", fontWeight: "bold", color: "#10b981" }}>{approvedApps}</div>
            </div>
            <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)" }}>
              <div style={{ fontSize: "0.85em", color: "#6b7280", marginBottom: "8px", fontWeight: "600" }}>Rejected</div>
              <div style={{ fontSize: "clamp(1.8em, 8vw, 2.5em)", fontWeight: "bold", color: "#ef4444" }}>{rejectedApps}</div>
            </div>
          </div>
          ...
          <div style={{ background: "white", padding: "20px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)", border: "1px solid rgba(0,0,0,0.05)", marginBottom: "24px" }}>
            ...
            {recentApps.length === 0 ? (
              <p style={{ color: "#9ca3af", textAlign: "center", padding: "20px 0" }}>No applications yet.</p>
            ) : (
                  ...
                  return (
                    <div key={app.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px", backgroundColor: "#f9fafb", borderRadius: "8px", border: "1px solid #e5e7eb", flexWrap: "wrap", gap: "8px" }}>
                      <div style={{ flex: "1 1 200px", minWidth: "0" }}>
                        <div style={{ fontWeight: "600", color: "#1f2937", fontSize: "clamp(0.9em, 3vw, 1em)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{app.studentName}</div>
                        <div style={{ fontSize: "0.85em", color: "#6b7280", overflow: "hidden", textOverflow: "ellipsis" }}>{job?.title || "Unknown Job"}</div>
                      </div>
